    ...
    const check = async () => {
        ...
        for (const result of results) {
            ...
            if (type === "ban") {
                ...
                await modlog({ guild, user: member.user, action: "BAN_END", actionmaker: (staff ? staff : staffId), reason: client.handleLanguages("BAN_EXPIRED", client, guildID), duration: new Date(expires).getTime() - new Date(createdAt).getTime() }, client);
            }
            else if (type === "mute") {
                if (!member) {
                    continue;
                ...
                if (client.guildsConfig.get(guildID).config.muteGetAllRoles) {
                    ...
                    for (const role of previousRoles) {
                        if (!member.guild.roles.cache.get(role))
                            previousRoles?.splice(previousRoles?.indexOf(role), 1);
                    }
