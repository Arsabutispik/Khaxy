...
export default async (data, client) => {
  ...
  if (action === "WARNING") {
    message += replaceMassString(
      JSON.parse(JSON.stringify(client.handleLanguages("MODLOG_WARNING", client, guild.id))),
      {
    ...
    message += replaceMassString(JSON.parse(JSON.stringify(client.handleLanguages("MODLOG_BAN", client, guild.id))), {
      ...
      "{actionmaker_id}": actionmaker.id,
      "{reason}": reason,
      "{ban}": client.emojis.cache.get(client.config.Emojis.ban)?.toString() || "🔨",
    });
  } else if (action === "KICK") {
    ...
    message += replaceMassString(
      JSON.parse(JSON.stringify(client.handleLanguages("MODLOG_FORCED_BAN", client, guild.id))),
      {
        "{user_id}": user.id,
        "{actionmaker_username}": actionmaker.username,
        "{actionmaker_id}": actionmaker.id,
    ...
    message += replaceMassString(
      JSON.parse(JSON.stringify(client.handleLanguages("MODLOG_TIMED_BAN", client, guild.id))),
      {
        "{user_username}": user.username,
        "{user_id}": user.id,
        "{actionmaker_username}": actionmaker.username,
    ...
    message += replaceMassString(
      JSON.parse(JSON.stringify(client.handleLanguages("MODLOG_BAN_TIMEOUT", client, guild.id))),
      {
        "{user_id}": user.id,
        "{actionmaker_username}": actionmaker.username,
        "{actionmaker_id}": actionmaker.id,
    ...
    message = replaceMassString(
      JSON.parse(JSON.stringify(client.handleLanguages("MODLOG_CHANGES", client, guild.id))),
      {
    ...
    message += replaceMassString(
      JSON.parse(JSON.stringify(client.handleLanguages("MODLOG_TIMEOUT", client, guild.id))),
      {
        "{user_username}": user.username,
        "{user_id}": user.id,
        "{actionmaker_username}": actionmaker.username,
  ...
  try {
    ...
    log(
      "ERROR",
      `${__filename}`,
      `An error occurred while sending modlog message to ${guild.name} (${guild.id}): ${err.message}`,
    );
  }
  if (action !== "CHANGES") {
