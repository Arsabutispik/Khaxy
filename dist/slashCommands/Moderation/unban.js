  ...
  execute: async ({ interaction, client }) => {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      await interaction.reply({
        content: client.handleLanguages("UNBAN_USER_NOT_ENOUGH_PERMISSIONS", client, interaction.guildId),
        ephemeral: true,
      });
      return;
    }
    if (!interaction.guild.members.me.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      await interaction.reply({
        content: client.handleLanguages("UNBAN_NO_APP_PERMISSON", client, interaction.guildId),
        ephemeral: true,
      });
      return;
    }
    ...
    const reason =
      ...
      client.handleLanguages("UNBAN_NO_REASON", client, interaction.guildId);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        replaceMassString(
          JSON.parse(JSON.stringify(client.handleLanguages("UNBAN_SUCCESS", client, interaction.guildId))),
          {
