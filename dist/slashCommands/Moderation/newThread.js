...
export default {
    ...
    execute: async ({ interaction, client }) => {
        ...
        if (user.bot)
            return interaction.reply({ content: client.handleLanguages("NEWTHREAD_BOT_USER", client, interaction.guildId), ephemeral: true });
        if (user.id === interaction.user.id)
            return interaction.reply({ content: client.handleLanguages("NEWTHREAD_SAME_USER", client, interaction.guildId), ephemeral: true });
        if (await openMailsSchema.exists({ userID: user.id }))
            return interaction.reply({ content: client.handleLanguages("NEWTHREAD_ALREADY_OPEN", client, interaction.guildId), ephemeral: true });
        ...
        try {
            ...
            messages.push(`[${new Date().toLocaleString([lang, "en-US"], { timeZone: "UTC" })}] [TO USER] [${interaction.user.username}] (${_.startCase(_.capitalize(interaction.member.roles.highest.name))}) ${interaction.user.username}: ${message}`);
            await threadChannel.send(`**(${_.startCase(_.capitalize(interaction.member.roles.highest.name))}) ${interaction.user.username}:** ${message}`);
...
//# sourceMappingURL=newThread.js.map