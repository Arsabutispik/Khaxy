...
export default {
  ...
  execute: async ({ interaction, client }) => {
    ...
    try {
      ...
      messages.push(
        `[${new Date().toLocaleString([lang, "en-US"], { timeZone: "UTC" })}] [TO USER] [${interaction.user.username}] (${_.startCase(_.capitalize(interaction.member.roles.highest.name))}) ${interaction.user.username}: ${message}`,
      );
      await threadChannel.send(
        `**(${_.startCase(_.capitalize(interaction.member.roles.highest.name))}) ${interaction.user.username}:** ${message}`,
      );
