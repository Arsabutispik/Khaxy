...
export default {
    ...
    execute: async ({ interaction, client }) => {
        ...
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageRoles))
            return interaction.reply({ content: client.handleLanguages("UNMUTE_USER_NOT_ENOUGH_PERMISSIONS", client, interaction.guildId), ephemeral: true });
        const data = await punishmentSchema.findOne({ guildID: interaction.guild.id, userID: member.id, type: "mute" });
        if (!data)
            return interaction.reply({ content: client.handleLanguages("UNMUTE_NOT_MUTED", client, interaction.guildId), ephemeral: true });
        ...
        if (client.guildsConfig.get(interaction.guild.id).config.muteGetAllRoles) {
            if (!data.previousRoles)
                return interaction.reply({ content: client.handleLanguages("UNMUTE_NO_PREVIOUS_ROLES", client, interaction.guildId), ephemeral: true });
