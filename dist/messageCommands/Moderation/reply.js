...
export default {
  ...
  async execute({ client, message, args }) {
    ...
    if (!message.member.permissions.has(PermissionsBitField.Flags.ManageMessages, true))
    ...
    if (!(await openMailsSchema.exists({ channelID: message.channel.id })))
      return message.channel.send(client.handleLanguages("REPLY_NOT_A_MAIL_CHANNEL", client, message.guildId));
    ...
    if (msg.length > 2000)
      return message.channel.send(client.handleLanguages("REPLY_TOO_LONG", client, message.guildId));
    try {
      await user.send(msg);
      ...
      await openMailsSchema.updateOne(
        { channelID: message.channel.id },
        {
          $inc: { "messageCount.modMessageCount": 1 },
          $push: {
            messages: {
              $each: [
