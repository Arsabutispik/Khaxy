...
export async function logMessageDelete(message: Message<true>, guildConfig: GuildWithLogs) {
  ...
  try {
    ...
    logger.log({
      level: "error",
      message: `Failed to send message delete log for guild ${message.guild.name} (${message.guild.id})`,
      error,
    });
    return;
  }
  ...
  for (let i = 1; i < batches.length; i++) {
    ...
    try {
      ...
      logger.log({
        level: "error",
        message: `Failed to send batch ${i + 1} for message delete log in guild ${message.guild.name} (${message.guild.id})`,
        error,
      });
    }
  }
