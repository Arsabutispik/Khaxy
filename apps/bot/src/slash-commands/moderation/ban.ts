...
export default {
  ...
  async execute(interaction, guildConfig) {
    ...
    const t = client.i18next.getFixedT(guildConfig.language, "commands", "ban");
    ...
    const successType = dmSent ? "success" : "fail";
    try {
      await guild.members.ban(targetUser, { reason, deleteMessageSeconds: preserve ? 0 : 604800 });
    } catch (error) {
      logger.error({ message: "Error banning user", error });
      return interaction.editReply(t(($) => $.errors.banFailed, { user: targetUser.tag }));
    }
    await interaction.editReply({
      content: t(($) => $.message[successType][typeKey], {
        user: targetUser.tag,
        duration: longDuration,
    ...
    if (guildConfig.logConfig?.guildLogsChannelId) {
      await logBanAdd({ guild, user: targetUser, reason, guildConfig });
    }
  },
} as SlashCommandBase;
