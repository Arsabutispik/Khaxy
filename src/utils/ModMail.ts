...
export async function modMailLog(client: Client, channel: TextChannel, user: User | null, closer: User) {
  ...
  const attachment = await modMailTextFile(messages, mod_mail_messages, client, t, user);
 ...
 * @param mod_mail_messages - An array of mod mail messages fetched from the database.
 * @param client - The Discord client instance.
...
export async function modMailTextFile(
  messages: string[] = [],
  mod_mail_messages: Awaited<ReturnType<typeof getModMailMessages>> = [],
  client: Client,
  t: TFunction,
  user: User,
) {
