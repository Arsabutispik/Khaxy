  ...
  if (interaction.isChatInputCommand()) {
    if (interaction.channel!.type === ChannelType.DM)
      return interaction.reply({
        content: "Slash command unavailable in DMs!",
        ephemeral: true,
      });
    ...
    if (client.guildsConfig.get(interaction.guild!.id) === undefined) {
      const data = await guildConfig.findOne({
        guildID: interaction.guild!.id,
      });
      if (!data) {
        const newData = await guildConfig.findOneAndUpdate(
          { guildID: interaction.guild!.id },
          {},
    ...
    try {
      ...
      log(
        "SUCCESS",
        "Slash Command",
        `${interaction.user.tag} (${interaction.user.id}) executed ${interaction.commandName} in ${interaction.guild!.name} (${interaction.guild!.id})`,
      );
    } catch (error) {
