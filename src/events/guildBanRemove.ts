    ...
    await modlog(
      {
        ...
        reason: client.handleLanguages("BAN_EVENT_REMOVE_NO_AUDIT_PERMISSION", client, ban.guild.id),
      },
      client,
    );
    return;
    ...
    await modlog(
      {
        ...
        action: "BAN_REMOVE",
        actionmaker: client.user!,
        reason: client.handleLanguages("BAN_EVENT_REMOVE_COULDNT_FIND_EXECUTOR", client, ban.guild.id),
      },
      client,
    );
    return;
  ...
  await modlog(
    {
      ...
      reason: reason || client.handleLanguages("BAN_EVENT_REMOVE_NO_REASON_SPECIFIED", client, ban.guild.id),
    },
    client,
  );
};
