...
export default async (client: KhaxyClient, message: Message) => {
  ...
  if (message.channel.type === ChannelType.DM) {
    ...
    if (await openMailsSchema.exists({ userID: message.author.id })) {
      ...
      const threadChannel = guild.channels.cache.get(mail.channelID) as TextChannel;
      if (!threadChannel) {
      ...
      if (
        ...
        await message.author.send(
          "Your message is too long. Keep it under 2000 characters. Remember, your username will be added to the message.",
        );
        return;
      }
      ...
      const commonGuilds = client.guilds.cache.filter(
        async (guild) =>
          guild.members.cache.has(message.author.id) && client.guildsConfig.get(guild.id)?.config.modmail,
      );
      if (commonGuilds.size === 0) return;
      ...
      const actionRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(stringSelection);
      const msg = await message.author.send({
        content: "Select a server to send a modmail message to",
        components: [actionRow],
      });
      const filter = (interaction: SelectMenuInteraction) => interaction.user.id === message.author.id;
      try {
        ...
        const botThreadMessage = await threadChannel.send(
          replaceMassString(client.handleLanguages("MOD_MAIL_THREAD_CREATED", client, guild.id), {
        ...
        messages.push(
          `[${new Date().toLocaleString([lang, "en-US"], { timeZone: "UTC" })}] [FROM USER] [${message.author.username}] ${message.content}${message.attachments.size ? "\n" : ""}${message.attachments?.map((attachment) => attachment.url).join("\n")}`,
        );
        await threadChannel.send(
          `**[${message.author.username}]:** ${message.content}\n${message.attachments?.map((attachment) => attachment.url).join("\n")}`,
        );
        await collector.reply(
          guildConfig.config.modmail.newThreadMessage ||
            "Your modmail thread has been created. Please wait for a staff member to respond.",
        );
        if (process.env.npm_lifecycle_event !== "test") {
          await message.react(client.emojis.cache.get(client.config.Emojis.mailSent) || "✉️");
        }
        await threadChannel.send(
          `${client.emojis.cache.get(client.config.Emojis.gearSpinning) || "⚙️"} **${client.user!.username}:** ${guildConfig.config.modmail.newThreadMessage || "Your modmail thread has been created. Please wait for a staff member to respond."}`,
        );
        messages.push(
          `[${new Date().toLocaleString([lang, "en-US"], { timeZone: "UTC" })}] [BOT TO USER] ${guildConfig.config.modmail.newThreadMessage || "Your modmail thread has been created. Please wait for a staff member to respond."}`,
        );
        await openMailsSchema.findOneAndUpdate(
          {
            guildID: guild.id,
  ...
  if (
    ...
    message.channel.id === leaderboardChannel
  ) {
    const results = await bumpLeaderboardSchema.findOne({
    ...
    if (result && result.error) {
      message.channel.send({
        content: "An error occurred while updating the leaderboard. Please try again later.\nError: " + result.error,
      });
    }
  ...
  if (message.content.startsWith("!!") && (await openMailsSchema.exists({ channelID: message.channel.id }))) {
    ...
    if (!snippetMessage) return;
    const openMail = await openMailsSchema.findOne({
    ...
    try {
      ...
      await openMailsSchema.updateOne(
        { channelID: message.channel.id },
        {
          $push: {
            messages: {
  ...
  if (
    !cmd &&
    (await openMailsSchema.exists({ channelID: message.channel.id })) &&
    !message.author.bot &&
