...
import {
    ...
    Message,
    MessageComponentInteraction, ModalBuilder, ModalSubmitInteraction, PermissionsBitField,
    StringSelectMenuBuilder, TextChannel, TextInputBuilder, TextInputStyle
} from "discord.js";
...
export default async (client: HolyClient, message: Message) => {
    ...
    if(message.channel.type === ChannelType.DM) {
        if(!client.userTickets.has(message.author.id)) {
            try {
                if (message.content.length > 4096) return await message.reply("Your message can not be longer than 4096 characters!");
                ...
                if (commonGuilds.size > 0) {
                    ...
                    const filter = (messageFilter: MessageComponentInteraction) => messageFilter.user.id === message.author.id;
                    ...
                    if (stringSelect) {
                        ...
                        if (guild) {
                            ...
                            let d = new Date();
                            let padding = (v: number) => `0${v}`.slice(-2);
                            ...
                            const rowModal = new ActionRowBuilder<TextInputBuilder>()
                            ...
                            const filter = (messageFilter: MessageComponentInteraction) => {
                            ...
                            button.on("collect", async (button) => {
                                if (button.customId === "reject") {
                                    ...
                                    const modalFilter = (messageFilter: ModalSubmitInteraction) => {
                                        const member = guild.members.cache.get(messageFilter.user.id);
                                        if (member) {
                                            return member.permissions.has(PermissionsBitField.Flags.ManageGuild);
                                        } else {
                                    ...
                                    const rejectLog = new EmbedBuilder()
                                        ...
                                        .setColor("Red")
                                        .setTimestamp();
                                    const logChannel = guild.channels.cache.get(client.guildsConfig.get(guild.id)!.config.modmail.logChannel) as TextChannel;
                                    if (logChannel) {
                            ...
                            const filterMessage = (messageFilter: Message) => {
                            ...
                            collector.on("collect", async (staffResponse) => {
                                if (staffResponse.author.id === client.user!.id) return;
            ...
            let mailChannel: TextChannel;
            try {
                mailChannel = await client.channels.fetch(client.userTickets.get(message.author.id)!) as TextChannel;
            } catch {
                client.userTickets.delete(message.author.id);
            ...
            if(mailChannel) {
                ...
                button.on("collect", async (button) => {
                    if (button.customId === "close") {
                        ...
                        const lastEmbed = new EmbedBuilder()
                            ...
                            .setColor("Greyple")
                            .setTimestamp();
                        const logChannel = button.guild.channels.cache.get(client.guildsConfig.get(button.guildId!)!.config.modmail.logChannel) as TextChannel;
                        if(logChannel) {
                ...
                const collector = mailChannel.createMessageCollector({filter: (messageFilter: Message) => messageFilter.channel.id === mailChannel.id});
                collector.on("collect", async (staffResponse) => {
                    if (staffResponse.author.id === client.user!.id) return;
                    const MailEmbed = new EmbedBuilder()
                        .setAuthor({name: staffResponse.author.username, iconURL: staffResponse.author.displayAvatarURL()})
