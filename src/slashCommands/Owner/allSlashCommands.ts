...
export default {
    ...
    execute: async ({interaction, client}) => {
        if(interaction.user.id !== "903233069245419560") return interaction.reply({content: "Bu komutu sadece sahibim kullanabilir!", ephemeral: true})
        ...
        if(!id) return await interaction.reply({ephemeral: true, embeds: [embed]});
        const splitted = id.split(" ")
        const command = commands.filter(val =>  splitted.includes(val.id))
        if(!command) return interaction.reply({content: "Böyle bir komut bulunamadı!", ephemeral: true})
