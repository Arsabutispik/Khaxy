  ...
  execute: async ({ interaction, client }) => {
    if (interaction.user.id !== "903233069245419560")
      return interaction.reply({
        content: "Bu komutu sadece sahibim kullanabilir!",
        ephemeral: true,
      });
    const commands = Array.from((await client.application!.commands.fetch()).values()).concat(
      Array.from((await client.guilds.cache.get("1007285630427996292")!.commands.fetch()).values()),
    );
