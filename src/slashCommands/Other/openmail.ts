...
export default {
    ...
    async execute({interaction, client}) {
        ...
        // Do something with the message and attachment
        const data = client.guildsConfig.get(interaction.guild!.id);
        if (!data) return interaction.reply(client.handleLanguages("SERVER_HAS_NO_CONFIGURATION", client, interaction.guildId!))
        if (!data.config.modmail) return interaction.reply(client.handleLanguages("MAIL_CHANNEL_NOT_SET", client, interaction.guildId!))
        if(client.userTickets.has(interaction.user.id)) {
            console.log(client.userTickets.get(interaction.user.id))
            const channel = interaction.guild!.channels.cache.get(client.userTickets.get(interaction.user.id)!) as TextChannel;
            ...
            client.ticketMessages.set(channel.id, `[${new Date().toString()}] - ${interaction.user.username}(${interaction.user.id}): ${message} ${attachment ? `\n\nAttachment: ${attachment.url}` : ""}`);
            await openMailsSchema.findOneAndUpdate({
                guildID: interaction.guild!.id,
            }, {
                guildID: interaction.guild!.id,
