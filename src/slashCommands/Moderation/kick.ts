...
export default {
    ...
    execute: async ({interaction, client}) => {
        if(!interaction.guild!.members.me!.permissions.has(PermissionsBitField.Flags.KickMembers)) {
            await interaction.reply({content: client.handleLanguages("KICK_APP_NOT_ENOUGH_PERMISSIONS", client, interaction.guildId!), ephemeral: true})
            return
        }
        ...
        if(!(interaction.member as GuildMember).permissions.has(PermissionsBitField.Flags.KickMembers)) return interaction.reply({content: client.handleLanguages("KICK_USER_NOT_ENOUGH_PERMISSIONS", client, interaction.guildId!), ephemeral: true});
        if(targetMember.id === interaction.user.id) {
            await interaction.reply({content: client.handleLanguages("KICK_CANT_KICK_YOURSELF", client, interaction.guildId!), ephemeral: true})
            return
        }
        if(targetMember.user.bot) {
            await interaction.reply({content: client.handleLanguages("KICK_CANT_KICK_BOT", client, interaction.guildId!), ephemeral: true})
            return
        }
        if(targetMember.roles.highest.position >= (interaction.member as GuildMember)!.roles.highest.position) {
            await interaction.reply({content: client.handleLanguages("KICK_TARGET_HIGH_ROLE", client, interaction.guildId!), ephemeral: true})
            return
        }
        if(targetMember.permissions.has("KickMembers")) {
            await interaction.reply({content: client.handleLanguages("KICK_TARGET_PERMISSIONS", client, interaction.guildId!), ephemeral: true})
            return
        }
        try {
