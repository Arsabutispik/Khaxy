...
import { GuildMember, EmbedBuilder, SlashCommandBuilder, PermissionsBitField, ChannelType } from "discord.js";
import ms from "ms";
...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName("force")
        .setDescription("Force bans an user from the server")
  ...
  execute: async ({ interaction, client }) => {
    ...
    if (!interaction.guild!.members.me!.permissions.has(PermissionsBitField.Flags.BanMembers))
      return interaction.reply({
        content: client.handleLanguages("BAN_BOT_MISSING_PERMS", client, interaction.guild!.id),
        ephemeral: true,
      });
    ...
    if (!(interaction.member as GuildMember).permissions.has(PermissionsBitField.Flags.BanMembers))
      return interaction.reply({
        content: client.handleLanguages("BAN_USER_MISSING_PERMS", client, interaction.guild!.id),
        ephemeral: true,
      });
    if (subCommand === "member") {
      ...
      try {
        ...
        await interaction.reply({
          content: client.handleLanguages("BAN_USER_NOT_FOUND", client, interaction.guild!.id),
          ephemeral: true,
        });
        return;
      }
      ...
      if (targetMember.id === interaction.user.id) {
        await interaction.reply({
          content: client.handleLanguages("BAN_CANT_BAN_YOURSELF", client, interaction.guild!.id),
          ephemeral: true,
        });
        return;
      }
      if (targetMember.user.bot) {
        await interaction.reply({
          content: client.handleLanguages("BAN_CANT_BAN_BOT", client, interaction.guild!.id),
          ephemeral: true,
        });
        return;
      }
      if (targetMember.roles.highest.position >= (interaction.member as GuildMember)!.roles.highest.position) {
        await interaction.reply({
          content: client.handleLanguages("BAN_USER_HIGHER_ROLE", client, interaction.guild!.id),
          ephemeral: true,
        });
        return;
      }
      if (targetMember.permissions.has("BanMembers") || targetMember.roles.cache.hasAny(...data.config.staffRole)) {
        await interaction.reply({
          content: client.handleLanguages("BAN_USER_HAS_PERMS", client, interaction.guild!.id),
          ephemeral: true,
        });
        return;
      }
      if (interaction.options.getNumber("duration", false) && !interaction.options.getString("time", false)) {
        await interaction.reply({
          content: client.handleLanguages("BAN_SPECIFY_UNIT", client, interaction.guild!.id),
          ephemeral: true,
        });
        return;
      } else if (!interaction.options.getNumber("duration", false) && interaction.options.getString("time", false)) {
        await interaction.reply({
          content: client.handleLanguages("BAN_SPECIFY_TIME", client, interaction.guild!.id),
          ephemeral: true,
        });
        return;
      }
      if (interaction.options.getNumber("duration", false)) {
        if (interaction.options.getNumber("duration", false)! < 0) {
          await interaction.reply({
            content: client.handleLanguages("BAN_INVALID_TIME", client, interaction.guild!.id),
            ephemeral: true,
          });
          return;
        }
        const duration = ms(
          `${interaction.options.getNumber("duration", true)}${interaction.options.getString("time", true)}`,
        );
        ...
        try {
          await targetMember.send(
            replaceMassString(
              JSON.parse(JSON.stringify(client.handleLanguages("BAN_USER_DURATION_DM", client, interaction.guild!.id))),
              {
        ...
        if (interaction.guild!.channels.cache.get(data.config.modlogChannel)) {
          try {
            ...
            await interaction.followUp({
              content: client.handleLanguages("NO_PERMISSION_MODLOG", client, interaction.guild!.id),
              ephemeral: true,
            });
          }
        }
        ...
        try {
          await targetMember.send(
            replaceMassString(
              JSON.parse(JSON.stringify(client.handleLanguages("BAN_USER_DM", client, interaction.guild!.id))),
              {
                "{guild_name}": interaction.guild!.name,
                "{reason}": reason,
              },
          ...
          await interaction.reply(
            replaceMassString(
              JSON.parse(
                JSON.stringify(client.handleLanguages("BAN_USER_MESSAGE_FAIL", client, interaction.guild!.id)),
              ),
              {
                "{targetMember_username}": targetMember.user.tag,
                "{case}": data.case.toString(),
      ...
      if (interaction.options.getString("duration", false)) {
        const duration = ms(
          `${interaction.options.getString("duration", true) || "0"}${interaction.options.getString("time", true) || "s"}`,
        );
        let longduration = ms(duration!, { long: true });
        if (lang === "tr") {
          longduration = longduration
        ...
        await interaction.reply(
          replaceMassString(
            JSON.parse(
              JSON.stringify(client.handleLanguages("BAN_FORCE_DURATION_MESSAGE", client, interaction.guild!.id)),
            ),
            {
