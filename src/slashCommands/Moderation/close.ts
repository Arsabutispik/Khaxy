...
import { SlashCommandBuilder, PermissionsBitField, TextChannel, GuildMember, ChannelType } from "discord.js";
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        ...
        .setDescription(
          "The channel ID of the modmail to close. If not provided, it will try to close the current channel.",
        )
        .setDescriptionLocalizations({
  ...
  async execute({ client, interaction }) {
    ...
    if (!(interaction.member as GuildMember).permissions.has(PermissionsBitField.Flags.ManageMessages))
      return interaction.reply({
        content: client.handleLanguages("CLOSEMAIL_NO_PERMS", client, interaction.guild!.id),
        ephemeral: true,
      });
    ...
    if (!(await openMailsSchema.exists({ channelID: channel })))
      return interaction.reply({
        content: client.handleLanguages("CLOSEMAIL_NOT_MODMAIL", client, interaction.guild!.id),
        ephemeral: true,
      });
    const openMail = await openMailsSchema.findOne({ channelID: channel });
    await interaction.reply({
      content: client.handleLanguages("CLOSEMAIL_CLOSE", client, interaction.guild!.id),
    });
    const messages = openMail!.messages;
    const lang = guildConfig.config.language;
    messages.push(
      `[${new Date().toLocaleString([lang, "en-US"], { timeZone: "UTC" })}] [COMMAND] [${interaction.user.username}] ${interaction.commandName} ${interaction.options.getString("channelId") ? interaction.options.getString("channelId") : ""}`,
    );
    messages.push(
      `[${new Date().toLocaleString([lang, "en-US"], { timeZone: "UTC" })}] [BOT] ${client.handleLanguages("CLOSEMAIL_CLOSE", client, interaction.guild!.id)}`,
    );
    ...
    if (!logChannel) {
      try {
        ...
        await interaction.reply({
          content: client.handleLanguages("CLOSEMAIL_LOG_CHANNEL_CREATED", client, interaction.guild!.id),
          ephemeral: true,
        });
        messages.push(
          `[${new Date().toLocaleString([lang, "en-US"], { timeZone: "UTC" })}] [BOT] ${client.handleLanguages("CLOSEMAIL_LOG_CHANNEL_CREATED", client, interaction.guild!.id)}`,
        );
      } catch (error) {
    ...
    await fs.writeFile(
      `./logs/${openMail!.userID}.txt`,
      `# Modmail thread #${guildConfig.config.modmail.tickets} with ${mailUser.username} (${mailUser.id}) started at ${new Date(openMail!.createdAt).toLocaleString([lang, "en-US"], { timeZone: "UTC" })}. All times are in UTC+0.\n\n${messages.join("\n")}`,
    );
    //
    if (logChannel) {
      await logChannel.send({
        content: `Modmail thread #${guildConfig.config.modmail.tickets} with ${mailUser.username} (${mailUser.id}) was closed by ${interaction.user.username}\n**${openMail!.messageCount.userMessageCount}** message from the user, **${openMail!.messageCount.modMessageCount}** messages to the user and **${openMail!.messageCount.internalMessageCount}** internal chat messages.`,
        files: [`./logs/${openMail!.userID}.txt`],
      });
    }
    if (
      interaction
        .guild!.members.me?.permissionsIn(interaction.channel as TextChannel)
        .has(PermissionsBitField.Flags.ManageChannels)
    ) {
      interaction.channel!.delete();
    } else {
      interaction.channel!.send({
        content: client.handleLanguages("CLOSEMAIL_NO_PERM_TO_DELETE", client, interaction.guild!.id),
      });
    }
    try {
