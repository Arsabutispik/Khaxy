...
export default {
    ...
    execute: async ({interaction, client}) => {
        ...
        if(user.bot) return interaction.reply({content: client.handleLanguages("NEWTHREAD_BOT_USER", client, interaction.guildId!), ephemeral: true});
        if(user.id === interaction.user.id) return interaction.reply({content: client.handleLanguages("NEWTHREAD_SAME_USER", client, interaction.guildId!), ephemeral: true});
        if(await openMailsSchema.exists({userID: user.id})) return interaction.reply({content: client.handleLanguages("NEWTHREAD_ALREADY_OPEN", client, interaction.guildId!), ephemeral: true});
        const message = interaction.options.getString("message", true);
        try {
            const member = await interaction.guild!.members.fetch(user.id);
            const data = client.guildsConfig.get(interaction.guild!.id);
            if (!data) return interaction.reply(client.handleLanguages("SERVER_HAS_NO_CONFIGURATION", client, interaction.guild!.id));
            ...
            const botThreadMessage = await threadChannel.send(replaceMassString(client.handleLanguages("MOD_MAIL_THREAD_CREATED", client, interaction.guildId!), {
            ...
            await interaction.reply({content: client.handleLanguages("NEWTHREAD_SUCCESS", client, interaction.guildId!), ephemeral: true});
            messages.push(`[${new Date().toLocaleString([lang, "en-US"], {timeZone: "UTC"})}] [BOT] ${botThreadMessage.content}`)
            messages.push(`[${new Date().toLocaleString([lang, "en-US"], {timeZone: "UTC"})}] [TO USER] [${interaction.user.username}] (${_.startCase(_.capitalize((interaction.member as GuildMember).roles.highest.name))}) ${interaction.user.username}: ${message}`)
            await threadChannel.send(`**(${_.startCase(_.capitalize((interaction.member as GuildMember).roles.highest.name))}) ${interaction.user.username}:** ${message}`);
