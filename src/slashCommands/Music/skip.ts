...
export default {
  ...
  async execute({ client, interaction }) {
    ...
    async function skipVote(
      ...
      userStates: Collection<string, GuildMember>,
      player: GuildQueue,
    ) {
      if (!amount) {
        if (userStates.size === 1) {
          ...
          if (otherUser) {
            await interaction.reply({
              content: replaceMassString(
                JSON.parse(JSON.stringify(client.handleLanguages("SKIP_ONLY_ONE_USER", client, interaction.guildId!))),
                {
            ...
            const filter = (interaction: MessageComponentInteraction) =>
              ...
              (interaction.customId === "accept" || interaction.customId === "reject");
            try {
              ...
              switch (response.customId) {
                case "accept":
                  player.node.skip();
                  await msg.edit({
                    content: client.handleLanguages("SKIP_SUCCESS", client, interaction.guildId!),
                    components: [],
                  });
                  break;
                case "reject":
                  await msg.edit({
                    content: client.handleLanguages("SKIP_REJECT", client, interaction.guildId!),
                    components: [],
                  });
                  break;
              }
            } catch (e) {
              await handleErrors(client, e, "skip.ts", interaction);
              return;
            }
          }
        } else if (userStates.size >= 2) {
          const dataHolder: { id: string; voted: boolean; accepted: boolean }[] = [];
          let accepted = 0;
          const { embeds } = JSON.parse(
            JSON.stringify(client.handleLanguages("SKIP_VOTING_EMBED", client, interaction.guildId!)),
          );
          ...
          await interaction.reply({
            content: `${userStates.map((user) => user.toString()).join(", ")}`,
            embeds: embeds,
            components: [row],
          });
          const msg = await interaction.fetchReply();
          const filter = (interaction: MessageComponentInteraction) =>
            userStates.has(interaction.user.id) &&
            (interaction.customId === "accept" || interaction.customId === "reject");
          ...
          collector.on("collect", async (interaction: MessageComponentInteraction) => {
            if (interaction.customId === "accept") {
              ...
              if (index !== -1) {
                if (dataHolder[index].voted) {
                  await interaction.reply({
                    content: client.handleLanguages("SKIP_ALREADY_VOTED", client, interaction.guildId!),
                    ephemeral: true,
                  });
                  return;
                }
                ...
                embeds[0].fields![index].value = client.handleLanguages(
                  "SKIP_ACCEPTED_VOTE",
                  client,
                  interaction.guildId!,
                );
            ...
            if (dataHolder.find((data) => !data.voted)) return;
            collector.stop();
          });
        ...
        if (amount > player.size)
          return await interaction.reply({
            content: client.handleLanguages("SKIP_EXCEEDED_QUEUE_LENGTH", client, interaction.guildId!),
          });
        ...
        if (userStates.size === 1) {
          ...
          if (otherUser) {
            await interaction.reply({
              content: replaceMassString(
                JSON.parse(
                  JSON.stringify(client.handleLanguages("SKIP_ONLY_ONE_USER_AMOUNT", client, interaction.guildId!)),
                ),
                {
                  "{otherUser}": otherUser.toString(),
                  "{interactionUser}": interaction.user.username,
              ...
              switch (response.customId) {
                case "accept":
                  ...
                  await msg.edit({
                    content: replaceMassString(
                      JSON.parse(
                        JSON.stringify(client.handleLanguages("SKIP_AMOUNT_SUCCESS", client, interaction.guildId!)),
                      ),
                      {
                        "{amount}": amount.toString(),
                      },
                ...
                case "reject":
                  await msg.edit({
                    content: client.handleLanguages("SKIP_AMOUNT_REJECT", client, interaction.guildId!),
                    components: [],
                  });
                  break;
              }
          ...
          collector.on("end", async () => {
            if (userStates.size === 2 && accepted === 1) {
              ...
              await msg.edit({
                content: replaceMassString(
                  client.handleLanguages("SKIP_AMOUNT_SUCCESS", client, interaction.guildId!),
                  {
                    "{amount}": amount.toString(),
                  },
                ),
              ...
              await msg.edit({
                content: client.handleLanguages("SKIP_REJECTED_VOTE", client, interaction.guildId!),
                components: [],
              });
            }
            if (Math.floor((accepted / userStates.size) * 100) >= 60) {
    ...
    if (!player)
      return await interaction.reply({
        content: client.handleLanguages("BOT_NOT_PLAYING", client, interaction.guildId!),
      });
    if (!(interaction.member as GuildMember).voice.channel)
      return await interaction.reply({
        content: client.handleLanguages("USER_NOT_IN_VOICE", client, interaction.guildId!),
      });
    if (
      ...
      return await interaction.reply({
        content: client.handleLanguages("USER_NOT_IN_THE_SAME_VOICE", client, interaction.guildId!),
      });
    ...
    if (voiceStateUsers.size >= 1) {
      if (
        ...
        await interaction.reply({
          content: replaceMassString(
            JSON.parse(JSON.stringify(client.handleLanguages("SKIP_AMOUNT_SUCCESS", client, interaction.guildId!))),
            {
              "{amount}": amount.toString(),
            },
          )!,
