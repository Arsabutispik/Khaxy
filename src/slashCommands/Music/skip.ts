    ...
    async execute({ client, interaction}) {
        ...
        async function skipVote(interaction: ChatInputCommandInteraction, userStates: Collection<string, GuildMember>, player: GuildQueue) {
            if(!amount) {
                ...
                if (amount > player.size) return await interaction.reply({ content: "Kuyrukta o kadar şarkı yok." });
        ...
        if (!player) return await interaction.reply({ content: "Şu anda hiçbir şey çalmıyor." });
        if (!(interaction.member as GuildMember).voice.channel) return await interaction.reply({ content: "Bir ses kanalında olmanız gerekir." });
        if (interaction.guild!.members.me!.voice.channel && (interaction.member as GuildMember).voice.channel!.id !== interaction.guild!.members.me!.voice.channel.id) return await interaction.reply({ content: "Botla aynı ses kanalında olmanız gerekir." });
        if (!player.currentTrack) return await interaction.reply({ content: "Şu anda hiçbir şey çalmıyor." });
