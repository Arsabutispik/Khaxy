...
export default {
    ...
    async execute({ client, interaction}) {
        ...
        async function skipVote(interaction: ChatInputCommandInteraction, userStates: Collection<string, GuildMember>, player: GuildQueue) {
            if(!amount) {
                if(userStates.size === 1) {
                    ...
                    if(otherUser) {
                        ...
                        const filter = (interaction: MessageComponentInteraction) => interaction.user.id === otherUser.id && (interaction.customId === 'accept' || interaction.customId === 'reject');
                        try {
                    ...
                    let dataHolder: {id: string, voted: boolean, accepted: boolean}[] = [];
                    let accepted = 0;
                    ...
                    const msg = await interaction.fetchReply();
                    const filter = (interaction: MessageComponentInteraction) => userStates.has(interaction.user.id) && (interaction.customId === 'accept' || interaction.customId === 'reject');
                ...
                if (amount > player.size) return await interaction.reply({ content: client.handleLanguages("SKIP_EXCEEDED_QUEUE_LENGTH", client, interaction.guildId!) });
        ...
        if (!player) return await interaction.reply({ content: client.handleLanguages("BOT_NOT_PLAYING", client, interaction.guildId!)});
        if (!(interaction.member as GuildMember).voice.channel) return await interaction.reply({ content: client.handleLanguages("USER_NOT_IN_VOICE", client, interaction.guildId!)});
        if (interaction.guild!.members.me!.voice.channel && (interaction.member as GuildMember).voice.channel!.id !== interaction.guild!.members.me!.voice.channel.id) return await interaction.reply({ content: client.handleLanguages("USER_NOT_IN_THE_SAME_VOICE", client, interaction.guildId!)});
        if (!player.currentTrack) return await interaction.reply({ content: client.handleLanguages("BOT_NOT_PLAYING", client, interaction.guildId!)});
