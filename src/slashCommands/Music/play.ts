...
export default {
  help: {
    ...
    examples: [
      ...
      "play https://open.spotify.com/track/4cOdK2wGLETKBW3PvgPWqT?si=0b88f1a840684c49",
    ],
    category: "Müzik",
  },
  ...
  execute: async ({ client, interaction }) => {
    ...
    if (
      ...
      await interaction.reply({
        content: client.handleLanguages("PLAY_UNABLE_TO_SPEAK", client, interaction.guildId!),
        ephemeral: true,
      });
      return;
    }
    ...
    await interaction.reply(
      client
        ...
        .replace("{searchEmoji}", client.allEmojis.get(client.config.Emojis.searchEmoji)!.format),
    );
    ...
    const SongAddedEmbed = JSON.parse(
      JSON.stringify(client.handleLanguages("PLAY_EMBED", client, interaction.guildId!)),
    );
